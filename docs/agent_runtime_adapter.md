...
`codeagent.core.loop.agent_loop(runtime, messages, context)` 会原地修改 `messages`。Web 异步 run 使用不修改 core loop 的 streaming runner，调用 Anthropic `messages.stream(...)` 并通过 callback 交出 delta/tool events。Adapter 负责：

1. 将 MySQL messages 转换为 codeagent history。
2. 创建 codeagent runtime。
3. 调用非流式 `agent_loop` 或 streaming runner。
4. 对比调用前后的 history，提取新增 assistant message。
5. 提取或实时转发 tool_use 和 tool_result。
...
- `execute_run()`：获取 conversation lock、标记 `running`、调用 streaming adapter，边执行边写 delta/tool events，完成后写 assistant message。
